...
test('fieldviewloader.handleFieldRel', function(t) {
    ...
    fieldviewloader.handleFieldRel({ fieldRelUrl: 'test'}, { statusCode: 404 }, null,
                   function() {
...
test('fieldviewloader.handleFieldRel', function(t) {
    t.plan(1);
    sinon.stub(request, 'put').yields();
    fieldviewloader.handleFieldRel({ fieldRelUrl: 'test'}, { statusCode: 200 }, null,
                   function() {
...
test('fieldviewloader.updateMosaicBounds', function(t) {
    t.plan(3);
    var fieldUrl = 'http://test.com/api';
    var stub = sinon.stub(request, 'put').yields(null, null, null);
    var urls = { mosaic: true, fieldUrl: fieldUrl};
    var callback = sinon.spy();
    fieldviewloader.updateMosaicBounds(urls, callback);
    t.true(callback.getCall(0).args.length === 1, 'Calls callback with error with no bounds');
    urls.bounds = [];
    fieldviewloader.updateMosaicBounds(urls, callback);
    t.true(stub.getCall(0).args[0] === fieldUrl, 'Calls request.put at fieldUrl');
    t.true(callback.getCall(1).args.length === 0, 'Calls callback with no error');
});

test('fieldviewloader.checkFieldExists', function(t) {
    t.plan(2);
    var urls = { fieldUrl: 'http://test.com/api' };
    var stub = sinon.stub(request, 'get').yields(null, { statusCode: 404 }, null);
    var callback = sinon.spy();
    fieldviewloader.checkFieldExists(urls, callback);
    t.true(callback.getCall(0).args[0] instanceof Error, 'Calls callback with error when 404');
    stub.yields(null, { statusCode: 200 }, null);
    callback.reset();
    fieldviewloader.checkFieldExists(urls, callback);
    t.true(callback.getCall(0).args.length === 0, 'Calls callback with no error');
});
